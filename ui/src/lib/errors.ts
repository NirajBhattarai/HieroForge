...
export function isUserRejected(err: unknown): boolean {
  ...
  return (
    /user rejected|user denied|rejected the request|cancelled|canceled/i.test(msg) ||
...
export function getFriendlyErrorMessage(err: unknown, context: 'quote' | 'transaction' | 'wallet'): string {
  if (isUserRejected(err)) return 'Transaction was cancelled.'
  const raw = getErrorMessage(err, '')
  if (!raw) return context === 'quote' ? 'Unable to get quote.' : context === 'wallet' ? 'Connection failed.' : 'Transaction failed.'

  // Contract / revert patterns
  if (/insufficient liquidity|not enough liquidity/i.test(raw)) return 'Not enough liquidity for this amount. Try a smaller amount or add liquidity.'
  if (/pool does not exist|pool not found|invalid pool/i.test(raw)) return 'Pool not found. Create the pool first or check token addresses.'
  if (/slippage|slippage tolerance|amount out of range/i.test(raw)) return 'Price moved. Try again or increase slippage.'
