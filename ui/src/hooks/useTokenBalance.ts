...
function tokenIdToAddress(tokenId: string): string {
  const m = String(tokenId).trim().match(/^(\d+)\.(\d+)\.(\d+)$/)
  if (!m) return ''
...
export function useTokenBalance(
  ...
  decimals: number
): { balanceFormatted: string; balanceWei: bigint; loading: boolean; error: string | null } {
  const [balanceWei, setBalanceWei] = useState<bigint>(0n)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const isTokenValid = !!tokenAddress && /^0x[0-9a-f]{40}$/i.test(tokenAddress.trim())
  const isOwnerValid = !!ownerAddress && (
  ...
  useEffect(() => {
    ...
    fetch(url)
      .then((res) => {
        if (cancelled) return Promise.resolve(null as { balance?: MirrorAccountBalance } | null)
        if (res.status === 404) return Promise.resolve(null)
        return res.json() as Promise<{ balance?: MirrorAccountBalance; account?: string } | null>
      })
      .then((data) => {
        if (cancelled) return
        if (!data) {
          ...
          if (typeof console !== 'undefined' && console.log) {
            console.log('[HTS balance] Mirror Node', { token: tokenAddress, owner, balanceWei: '0', balanceFormatted: '0', source: 'mirror', note: 'account not found or 404' })
          }
          return
        }
        ...
        if (!Array.isArray(tokens)) {
          setBalanceWei(0n)
          if (typeof console !== 'undefined' && console.log) {
            console.log('[HTS balance] Mirror Node', { token: tokenAddress, owner, balanceWei: '0', balanceFormatted: '0', source: 'mirror' })
          }
          return
        }
        ...
        for (const t of tokens) {
          ...
          if (addr && addr === tokenNorm) {
            const raw = t.balance != null ? (typeof t.balance === 'string' ? BigInt(t.balance) : BigInt(Number(t.balance))) : 0n
            setBalanceWei(raw)
            if (typeof console !== 'undefined' && console.log) {
              const formatted = formatUnits(raw, decimals)
              console.log('[HTS balance] Mirror Node', { token: tokenAddress, owner, balanceWei: String(raw), balanceFormatted: formatted, decimals, source: 'mirror' })
            }
            return
          }
        }
      ...
      .catch((err) => {
        if (!cancelled) {
          ...
          if (typeof console !== 'undefined' && console.warn) {
            console.warn('[HTS balance] Mirror Node failed', { token: tokenAddress, owner, error: err })
          }
        }
      })
      .finally(() => {
