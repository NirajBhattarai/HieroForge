...
export function HashPackProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (!projectId || projectId === "your_project_id_here") {
      setError(
        "Missing NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID. Add it to .env (see .env.example).",
      );
      return;
    }
    ...
    hashconnect
      ...
      .then(() => {
        ...
        if ((hashconnect?.connectedAccountIds?.length ?? 0) > 0) {
          setAccountId(
            hashconnect.connectedAccountIds?.[0]?.toString() ?? null,
          );
        } else {
      ...
      .catch((err: unknown) => {
        setError(
          err instanceof Error ? err.message : "Failed to initialize HashPack",
        );
