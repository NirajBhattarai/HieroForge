...
async function main() {
  ...
  if (!usdcAddr || !forgeAddr) {
    console.error(
      "Set token addresses: QUOTE_TOKEN_USDC and QUOTE_TOKEN_FORGE (or TOKEN_ADDRESSES in constants)",
    );
    process.exit(1);
  }
  ...
  console.log(
    "Quote: %s USDC in → ? FORGE out (zeroForOne=%s)",
    AMOUNT_USDC_HUMAN,
    zeroForOne,
  );
  ...
  try {
    ...
    console.log(
      "Result: %s USDC → %s FORGE",
      AMOUNT_USDC_HUMAN,
      amountOutHuman,
    );
  } catch (e: unknown) {
    const err = e as {
      cause?: { cause?: { data?: string }; data?: string };
      data?: string;
    };
    ...
    if (hex.startsWith("0x7a5ed734")) {
      ...
      console.error(
        "The pool does not have enough liquidity to swap 100 USDC in one go.",
      );
      console.error(
        "Try a smaller amount (e.g. 1 USDC) or add more liquidity to the pool.",
      );
    } else {
      console.error("Quote failed. Revert data:", hex || "(none)");
      console.error(
        "Pool must exist with fee=3000, tickSpacing=60 and have liquidity.",
      );
    }
    throw e;
  }
