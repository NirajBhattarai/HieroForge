...
if (!currency0Input || !currency1Input) {
  console.error('Usage: register-pool.js --currency0 0x... --currency1 0x... --fee 3000 --tickSpacing 60 --symbol0 TKA --symbol1 TKB');
  process.exit(1);
}

// Sort currencies so currency0 < currency1
const sorted0 = currency0Input < currency1Input ? currency0Input : currency1Input;
const sorted1 = currency0Input < currency1Input ? currency1Input : currency0Input;

// Compute pool ID (matches Solidity PoolId.toId)
const encoded = encodeAbiParameters(
  parseAbiParameters('address, address, uint24, int24, address'),
  [sorted0, sorted1, fee, tickSpacing, '0x0000000000000000000000000000000000000000']
);
  ...
  try {
    ...
    if (err.name === 'ResourceNotFoundException') {
      ...
      await client.send(new CreateTableCommand({
        ...
        AttributeDefinitions: [{ AttributeName: 'poolId', AttributeType: 'S' }],
...
async function main() {
  ...
  console.log(`[DynamoDB] Pool registered: ${symbol0 || sorted0}/${symbol1 || sorted1} (fee=${fee})`);
